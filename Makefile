...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=asset-chain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=assetd \
...
draw-deps:
	...
	@goviz -i ./cmd/assetd -d 2 | dot -Tpng -o dependency-graph.png
...
build-docker-assetdnode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/assetd/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/assetd:Z tendermint/assetdnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
