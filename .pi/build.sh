...
# docker buildx build --tag asset --file .pi/Dockerfile --platform linux/arm64 --cache-from asset:cache --cache-to asset:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar asset
...
sudo bash -c "echo asset > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/asset.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/asset.img
